...
export async function pollMailboxOnce(): Promise<{ ingested: number; reason?: string }> {
  const connection = await getActiveMailboxConnection();
  if (!connection) {
    return { ingested: 0, reason: "no active mailbox connection" };
  }
  ...
  if (!clientId || !clientSecret) {
    ...
    return { ingested: 0, reason: "missing OAuth credentials" };
  }
  ...
  try {
    ...
    return { ingested: 0, reason: "refresh token retrieval failed" };
  }
  ...
  try {
    ...
    return { ingested: 0, reason: "token refresh failed" };
  }
  ...
  try {
    ...
    return { ingested: 0, reason: "message fetch failed" };
  }
