    ...
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-8">
      <section className="relative overflow-hidden rounded-2xl border bg-card px-5 py-8 shadow-sm sm:px-8 sm:py-10">
        <div className="relative max-w-2xl">
          ...
          <p className="mb-2 text-xs font-semibold tracking-[0.18em] text-primary uppercase">Support workspace</p>
          <h1 className="font-heading text-3xl font-semibold tracking-tight sm:text-4xl">Keep every customer conversation moving.</h1>
      ...
      <section>
        <div className="mb-4 flex items-end justify-between gap-4">
          <div>
            <h2 className="font-heading text-lg font-semibold">Jump back in</h2>
        ...
        <div className="grid gap-3 md:grid-cols-3">
          {visibleDestinations.map(({ href, label, description, icon: Icon }) => (
            <Link key={href} href={href} className="group rounded-xl border bg-card p-5 shadow-sm transition-colors hover:border-primary/50 hover:bg-primary/[0.03] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring">
